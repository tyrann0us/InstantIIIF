...
mw.loader.using( 'mediawiki.Title' ).then( function () {
    ...
    if ( document.querySelector( '#file img[data-iiif-title]' ) ) {
        ...
        mw.Title.prototype.getExtension = function () {
            ...
            if ( typeof this.getNamespaceId === 'function'
        ...
        while ( node && node !== document.body ) {
            if ( node.tagName === 'IMG' ) {
                if ( node.hasAttribute( 'data-iiif-title' ) ) {
                    const titleAttr = node.getAttribute( 'data-iiif-title' );
                    const pageAttr = node.getAttribute( 'data-iiif-page' );
                    const fullUrlAttr = node.getAttribute( 'data-iiif-full-url' );
                    if ( titleAttr ) {
                ...
                } else if ( node.classList.contains( 'mw-file-element' )
    ...
    if ( hasIiifImageOnPage ) {
        mw.loader.using( 'mmv' ).then( function () {
            ...
            if ( reuse.Share && reuse.Share.prototype && reuse.Share.prototype.set ) {
                ...
                reuse.Share.prototype.set = function ( img ) {
                    ...
                    const current = this.$pageInput && this.$pageInput.val
                        ? this.$pageInput.val()
                        : '';
                ...
                reuse.Download.prototype.set = function ( img ) {
                    ...
                    if ( isCurrentImageIiif
                        ...
                        this.image = Object.assign(
                            Object.create( Object.getPrototypeOf( this.image ) ),
                            this.image,
                        ...
                        if ( typeof this.handleSizeSwitch === 'function' ) {
                            try { this.handleSizeSwitch(); } catch ( e ) {}
                        }
                    }
                };
            }
            ...
            if ( reuse.EmbedFileFormatter
                ...
                EFF.prototype.getThumbnailHtml = function ( info, imgUrl, width, height ) {
                    let out = origGetHtml.call( this, info, imgUrl, width, height );
                    if ( isCurrentImageIiif ) {
                        out = out.replace(
                            /href=("|')([^"']*?)\1/g,
                            function ( match, quote, url ) {
                                return 'href=' + quote + rewriteIiifShareUrl( url ) + quote;
                            }
                        );
                    }
                    return out;
            ...
            if ( reuse.EmbedFileFormatter
                ...
                EFF.prototype.getThumbnailWikitext = function ( title, width, caption, alt ) {
                    let out = origGetWikitext.call( this, title, width, caption, alt );
                    if ( !isCurrentImageIiif ) {
                        return out;
                    }
    ...
    $( document ).on( 'mmv-metadata', function ( e ) {
        ...
        currentIiifFullUrl = image.thumbnail.getAttribute( 'data-iiif-full-url' ) || null;
        ...
        if ( !thumbnailInfoPatched && image.thumbnail.hasAttribute( 'data-iiif-page' ) ) {
