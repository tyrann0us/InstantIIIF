...
test.describe( 'File detail page (single-page IIIF)', () => {
	test( 'file page loads and shows the IIIF image', async ( { page } ) => {
		...
		await expect( mainImg ).toHaveAttribute(
			'data-iiif-title',
			/File:.*\.jpg/
		);
	} );

	test( 'shared-upload notice links to provider, not back to wiki', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/File:Df_dk_0007450' );
	...
	test( 'file history section is hidden for IIIF files', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/File:Df_dk_0007450' );
	...
	test( 'extended metadata shows attribution from manifest', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/File:Df_dk_0007450' );
