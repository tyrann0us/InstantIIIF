...
test.describe( 'Multi-page IIIF documents', () => {
	test( 'article thumbnails for different pages link to file page with ?page=N', async ( {
		page,
	} ) => {
		...
		const page2Link = page.locator(
			'a.mw-file-description[href*="page=2"]'
		);
		const page3Link = page.locator(
			'a.mw-file-description[href*="page=3"]'
		);
		await expect( page2Link ).toBeVisible();
		await expect( page3Link ).toBeVisible();
	} );

	test( 'each page thumbnail has data-iiif-page attribute', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
		...
		for ( let i = 0; i < count; i++ ) {
			pages.push(
				await images.nth( i ).getAttribute( 'data-iiif-page' )
			);
		}
	...
	test( 'page 2 thumbnail has data-iiif-full-url attribute', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
	...
	test( 'prev/next navigation thumbnails are marked with data-iiif-navigate', async ( {
		page,
	} ) => {
		...
		if ( count > 0 ) {
			...
			await expect( firstParent ).not.toHaveClass(
				/mw-file-description/,
				{ timeout: 10_000 }
			);
			...
			for ( let i = 1; i < count; i++ ) {
				...
				const className =
					( await parentLink.getAttribute( 'class' ) ) || '';
				expect( className ).not.toContain( 'mw-file-description' );
			}
		}
	} );

	test( 'file detail page main image links to correct page IIIF URL', async ( {
		page,
	} ) => {
	...
	test( 'MMV download button URL uses the displayed page', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
		...
		await expect( page.locator( '.mw-mmv-image' ) ).toBeVisible( {
			timeout: 10_000,
		} );
		...
		await expect
			.poll( async () => downloadBtn.getAttribute( 'href' ), {
				timeout: 10_000,
			} )
			.toContain( 'bsb11610364_00002' );
	} );

	test( 'MMV "More details" button URL includes ?page=N for multi-page', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
		...
		// strings).
		await expect
			.poll( async () => moreBtn.getAttribute( 'href' ), {
				timeout: 10_000,
			} )
			.toMatch(
	...
	test( 'MMV share URL is full local URL with ?page=N and #/media/ fragment', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
	...
	test( 'Wikitext embed contains |page=N for multi-page documents', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
	...
	test( 'download dialog "Original" URL targets the current canvas', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
	...
	test( 'download dialog attribution includes the original work URL', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/Kornhaus_Mehrseitig' );
	...
	test( 'clicking the displayed image in MMV navigates to the current canvas, not page 1', async ( {
		page,
	} ) => {
		...
		const navigationPromise = page.waitForRequest(
			( req ) =>
				/bsb11610364_\d{5}.*\/full\/full\/0\/default\.jpg/.test(
					req.url()
	...
	test( 'file detail page main image carries mw-file-description so MMV intercepts clicks', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/File:Df_dk_multipage?page=2' );
		...
		const navParents = page
			.locator( 'img[data-iiif-navigate="1"]' )
			.locator( 'xpath=..' );
		const count = await navParents.count();
		for ( let i = 0; i < count; i++ ) {
			const cls =
				( await navParents.nth( i ).getAttribute( 'class' ) ) || '';
			expect( cls ).not.toContain( 'mw-file-description' );
		}
	} );

	test( 'file detail page shows MMV "Open in Media Viewer" button for extension-less IIIF titles', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/File:Df_dk_multipage?page=2' );
	...
	test( '"Original file" link on detail page targets correct canvas', async ( {
		page,
	} ) => {
		await page.goto( '/wiki/File:Df_dk_multipage?page=2' );
